-- Keep block visibility checks set-based on the high-traffic feed and public
-- profile projection paths. The helper remains the narrow authorization
-- boundary for direct table/RLS consumers.
...
blocked_users as materialized (
  select block.blocked_user_id as user_id
  from public.user_blocks as block
  cross join viewer_context
  where block.blocker_id = viewer_context.viewer_id

  union

  select block.blocker_id as user_id
  from public.user_blocks as block
  cross join viewer_context
  where block.blocked_user_id = viewer_context.viewer_id
)
select
  profile.user_id,
...
where profile.is_public
  ...
  and not exists (
    select 1
    from blocked_users
    where blocked_users.user_id = profile.user_id
  );

drop function if exists public.list_community_feed_page(text, integer);

create function public.list_community_feed_page(
  ...
  visible_posts as (
    ...
    where viewer_context.viewer_id is not null
      ...
      and not exists (
        select 1
        from blocked_users
        where blocked_users.user_id = community_post.author_id
      )
      and (
    ...
    (
      ...
      where comment.post_id = feed_post.id
        ...
        and not exists (
          select 1
          from blocked_users
          where blocked_users.user_id = comment.author_id
        )
    ) as comments_count,
    (
      select count(*)
